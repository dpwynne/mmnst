...
plot.basics<-ggplot(ISI.data,aes(x=Time,y=Log.Diff))+theme_classic()+theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank(),plot.margin=unit(c(5.08,5.08,5.08,5.08),"mm"),axis.text=element_text(size=24),axis.line.x=element_line(size=0.5),axis.line.y=element_line(size=0.5))
plot.ticks<-plot.basics+scale_x_continuous(breaks=seq(floor(min(ISI.data$Time)),ceiling(max(ISI.data$Time)),length=6))

plot.ISI<-plot.ticks+geom_point(aes(x=Time,y=Log.Diff),size=0.2)
